...
parser.add_argument("--noCrop", default=False, action="store_true", help="Flag to turn off auto-cropping of images during processing (does not modify the original image)")
...
for item in os.listdir(): # loop through items in dir
    if item.endswith('.zip'): # check for ".zip" extension
        ...
        path = ntpath.basename(file_name)[0:-4]
        print(path)
                            ...
                            if(int(math.log10(int(text.description))) + 1 > 1 and int(math.log10(int(text.description))) + 1 < numDigits):
                                ...
                                for pos in matches_positions:
                                    ...
                                    while previous[upper] != "_" and upper < len(previous):
                                        upper+= 1
                                    appendToFile+= previous[lower:upper]
